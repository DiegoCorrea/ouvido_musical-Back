...
urlpatterns = [
  ...
  url(r'^users/(?P<user_id>([a-zA-Z]|[0-9])+)/recommendations/(?P<song_id>([a-zA-Z]|[0-9])+)/$', views.UserMusicRecommendation, name='User Musica Recommendation'),
  url(r'^users/(?P<user_id>([a-zA-Z]|[0-9])+)/recommendations/(?P<song_id>([a-zA-Z]|[0-9])+)/like/$', views.UserLikeMusicRecommendation, name='User Like Music'),
